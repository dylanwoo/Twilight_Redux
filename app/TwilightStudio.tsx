...
const TRANSIENT_MIN_INTERVAL_SECONDS = 65;
const TRANSIENT_MAX_INTERVAL_SECONDS = 140;
...
function drawCirrusWisps(
  ...
  const wispCount = 4 + Math.floor(random() * 4);
  ...
  for (let wisp = 0; wisp < wispCount; wisp += 1) {
    const startX = (random() * 0.6 - 0.22) * width;
...
function drawHorizonClouds(
  ...
  for (let cloud = 0; cloud < cloudCount; cloud += 1) {
    const centerX = (random() * 1.12 - 0.06) * width;
    ...
    drawSoftEllipse(
      ...
      cloudWidth * 0.52,
      cloudHeight * 2.1,
      `rgb(8 15 27 / ${opacity + 0.07})`,
      `rgb(24 35 51 / ${opacity})`,
    );
    drawSoftEllipse(
      context,
      centerX - cloudWidth * (0.08 + random() * 0.08),
      centerY - cloudHeight * 0.42,
      cloudWidth * (0.24 + random() * 0.15),
      cloudHeight * 0.72,
      `rgb(64 76 96 / ${opacity * 0.28})`,
      `rgb(18 27 42 / ${opacity * 0.54})`,
    );
    drawSoftEllipse(
      context,
      centerX + cloudWidth * (0.1 + random() * 0.1),
      centerY + cloudHeight * 0.34,
      cloudWidth * (0.18 + random() * 0.13),
      cloudHeight * 0.58,
      `rgb(76 48 53 / ${opacity * 0.15})`,
      `rgb(24 24 35 / ${opacity * 0.32})`,
    );
  }
  context.restore();
}
...
function drawRealisticClouds(
  ...
  drawCirrusWisps(context, width, height, groundLine, seed);
  drawHorizonClouds(context, width, height, groundLine, seed);
}
...
function drawRealisticSky(
  ...
  const twilightLine = mix(horizon, { r: 248, g: 177, b: 116 }, 0.76);
  const horizonFade = mix(twilightLine, { r: 35, g: 35, b: 49 }, 0.7);
  const groundLine = height * (0.84 - transition * 0.09);
  const horizonPosition = groundLine / height;
  const gradient = context.createLinearGradient(0, 0, 0, height);
  gradient.addColorStop(0, rgb(space));
  gradient.addColorStop(0.42, rgb(zenith));
  gradient.addColorStop(0.67, rgb(lowerSky));
  gradient.addColorStop(
    clamp(horizonPosition - 0.075, 0.71, 0.8),
    rgb(duskBlue),
  );
  gradient.addColorStop(
    clamp(horizonPosition - 0.014, 0.77, 0.86),
    rgb(twilightLine),
  );
  gradient.addColorStop(
    clamp(horizonPosition + 0.012, 0.8, 0.89),
    rgb(horizonFade),
  );
  ...
  glow.addColorStop(0, rgb(twilightLine, 0.16));
  glow.addColorStop(0.34, rgb(duskBlue, 0.07));
  ...
  drawRealisticClouds(context, width, height, groundLine, settings.seed);
  ...
  while (eventStart <= timeSeconds && eventIndex < 10_000) {
    ...
    const kind: TransientKind =
      kindSelector < 0.47
        ...
        : kindSelector < 0.76
          ? "plane"
          : "comet";
    const duration = TRANSIENT_DURATIONS[kind];
    if (timeSeconds < eventStart + duration) {
      return {
        ...
        altitude:
          kind === "comet"
            ? 0.34 + hashUnit(seed, eventIndex, 0x3956c25b) * 0.18
            : 0.22 + hashUnit(seed, eventIndex, 0x3956c25b) * 0.3,
...
function drawTransientEvent(
  ...
  if (event.kind === "satellite") {
    ...
    trail.addColorStop(0, "rgb(216 230 255 / 0)");
    trail.addColorStop(1, "rgb(238 245 255 / 0.34)");
    ...
    context.lineTo(x, y);
    context.stroke();
    context.shadowColor = "rgb(220 235 255 / 0.9)";
    context.shadowBlur = 7 * scale;
    context.fillStyle = "rgb(248 251 255 / 0.95)";
    context.beginPath();
    context.arc(x, y, 0.9 * scale, 0, Math.PI * 2);
    context.fill();
...
export function TwilightStudio() {
  ...
  const skySettings = useMemo<SkySettings>(
    ...
    [brightStars, horizon, paletteKey, realisticMode, seed, starShape, stars],
  );
          ...
          <section className="settings-section" aria-labelledby="appearance-title">
            ...
            <div className="control-stack">
              <label className="twinkle-control realistic-control">
                <span>
                  <span className="control-label">Realistic mode</span>
              ...
              <label className="control-row">
                ...
                {realisticMode ? (
                  ...
                  >
                    Realistic mode uses natural optical star profiles.
                  </span>
                ) : null}
              </label>
